...
def neighbour_coords(x, y, w, h):
    ...
    for _x in range(-1, 2):
        for _y in range(-1, 2):
            ...
            if t_x == x and t_y == y:
                continue
            if (t_x >= 0 and t_y >= 0) and (t_x < w and t_y < h):
...
def neighbours(area, x, y):
    width = len(area[0])
    height = len(area)
    coords = neighbour_coords(x, y, width, height)
        ...
        for y, row in enumerate(area):
            ...
            for x, col in enumerate(row):
                if area[y][x] == "L" and neighbours(area, x, y) == 0:
                    new_col += "#"
                elif area[y][x] == "#" and neighbours(area, x, y) >= 4:
                    new_col += "L"
                else:
                    new_col += area[y][x]
